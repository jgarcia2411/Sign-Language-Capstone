...
#sentences, translations = translate(model, test_loader, device, dataset)
#for i in sentences:
...
def print_loss(train_loss, eval_loss):
    ...
    plt.legend()
    plt.savefig(OR_PATH+'/LOSS_class.png')
    #plt.show()
...
for epoch in range(num_epochs):
    ...
    if valid_loss < best_valid_loss:
        ...
        torch.save(model.state_dict(), 'model_{}.pt'.format('attention'))
        print("The model has been saved!")
        print(f'\tBest Train Loss: {train_loss:.3f}')
        print(f'\tBest Test Loss: {valid_loss:.3f}')


        sentences, translations = translate(model, test_loader, device, dataset) #dataset used for getting vocabulary
        print(f'Original sentences \n {sentences}')
        print(f' Translated Sentence \n {translations}')
        excel = pd.DataFrame()
        excel['Original Sentence'] = sentences
        excel['Translation'] = translations
        excel.to_excel(OR_PATH+'/attention_results_{}.xlsx'.format(epoch))
